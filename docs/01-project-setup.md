...
- [x] `app/(tabs)/_layout.tsx` を3タブ構成に変更
  - `index`（カレンダー＝ホーム）／`items`（一覧）／`settings`（設定）
  - タブラベルは日本語：「カレンダー」「一覧」「設定」
- [x] 各タブにプレースホルダー画面を作成（画面タイトルのみ表示）
- [x] `app.json` の `userInterfaceStyle` を `"light"` に変更（ダークモード非対応のため）
- [x] アプリ表示名を「のんだっけ？」に設定（`app.json` の `name`）
