...
- `app/(tabs)/` — 3タブ：`index.tsx`（カレンダー=ホーム。月表示＋サマリーカード＋カテゴリタブ＋日別詳細シート実装済み。記録トグルは `handleToggle` → `reload()` で即時反映。記録は `listRecords` で全件取得 — interval の導出が表示月外の最終服用日に依存するため月単位で取らない）／`items.tsx`（一覧。カテゴリ別セクション＋中止/再開＋ヘッダー「＋」導線。最終服用日は `getLastTakenDates` の GROUP BY 一括取得）／`settings.tsx`（設定。アプリ情報＋プライバシーポリシー導線＋免責事項＋データ全削除）
...
- `components/calendar/` — `summary-card.tsx`／`month-calendar.tsx`（自作月グリッド。外部カレンダーライブラリ禁止）／`legend.tsx`／`category-tabs.tsx`（全部/お薬/サプリ/その他の4等分タブ。カテゴリタブに色ドット常時表示）／`category-theme.ts`（`categoryFilterTheme` — カテゴリ連動の淡色テーマ。タブ・サマリーカード・今日セルが同じテーマで染まる。全部=薄ミント／お薬=濃ミント catMedicineLight で濃淡区別）／`day-detail-sheet.tsx`（RNコア Modal のボトムシート。表示専用でミューテーションは親が持つ。未来日は記録不可。今日のシートでは interval の飲み忘れを「N日遅れ」表示）。表示ロジックは `lib/schedule/calendar.ts` の純粋関数（buildMonthGrid / summarizeDayEntries / summarizeToday / formatDateLabel）に分離。日別スロット導出は `lib/schedule/derive.ts` の `deriveDayDetail`
...
- **予定日はDBに保存しない**：服用予定日は常に `intake_records` と `items` のスケジュール設定から導出する。interval型は「最終服用日＋N日」で自動でずれ、daily/weekly型はカレンダー固定。過去日の記録を追記・修正・削除しても再計算だけで整合する設計。interval型のカレンダー未来日は次回予定日を起点にN日周期の見込みを投影表示する
...
- Android はエッジトゥエッジ表示（SDK 54 で常時有効）。画面最下端まで届くスクロール内容・ボトムシートには `useSafeAreaInsets().bottom` を下パディングに足す（タブ3画面はタブバーが吸収するので不要）
